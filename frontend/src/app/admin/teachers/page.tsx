...
import { AlertCircle, FileSpreadsheet, Mail, MoreHorizontal, PowerOff, Upload } from "lucide-react";
...
export default function TeachersPage() {
  ...
  const handleImport = async () => {
    ...
    const result = await importUsers.mutateAsync({ role: "teacher", file: importFile });
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Button onClick={() => setImportOpen(true)}>
            <Upload className="mr-2 h-4 w-4" /> Add Teachers
          </Button>
        </div>
          ...
          {!importResult ? (
            <>
              <p className="text-sm text-muted-foreground">
                ...
                . Teachers are auto-assigned to the specified class and section.
              </p>
                ...
                {importResult.created > 0 && (
                  <div className="rounded-lg bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-800 p-3">
                    <p className="text-sm font-medium text-green-700 dark:text-green-300">
                      Successfully imported {importResult.created} teacher{importResult.created !== 1 ? "s" : ""}
                    </p>
                  </div>
                )}
